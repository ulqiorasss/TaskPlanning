...
namespace HabitTracker.App.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        private ObservableCollection<HabitTask> _tasks = new();
        ...
        {
            _currentPerson = person ?? throw new ArgumentNullException(nameof(person));
                ...
                EditTaskCommand = new RelayCommand(obj =>
            {
                ...
                if (selectedTask != null)
                {
                    ...
                    {
                        MessageBox.Show("Пожалуйста, выберите задачу из списка для редактирования!", "Внимание", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
            });
        ...
        private void LoadTasks()
        {
            try
            {
                IQueryable<HabitTask> query = _db.HabitTasks
                    .Include(t => t.Tags)
                    .Where(t => t.PersonalId == _currentPerson.Id);
            ...
            {
                MessageBox.Show($"Ошибка загрузки/фильтрации данных: {ex.Message}");
            }
        }
